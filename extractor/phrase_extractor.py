...
class PhraseExtractor(BaseExtractor):
    ...
    def extract(self, text: str, window=3, normalize=False, top_percent=None, topN=10):
        ...
        doc = self.nlp(text)
        nodes = [(Word(sent_id, token.i, token), token.tag_ in self.valid_tags)
                 for sent_id, sent in enumerate(doc.sents) for token in sent]
    ...
    def get_combined_keywords(self, doc: Doc, window, normalize):
        ...
        filtered_phrases = self.candidate_filtering(phrases,
                                                    stoplist=list(self.stop_words) + list(string.punctuation) + list(BRACKET_ESCAPES.keys()))
        return filtered_phrases

    def candidate_filtering(self, candidates: Iterable[Phrase], stoplist = None, minimum_length=3, maximum_ngrams=5,
                            valid_punctuation_marks='-', only_alphanum=True, pos_blacklist=None):
        ...
        for candidate in candidates:
            ...
            elif only_alphanum and not all([self._is_alphanum(w, valid_punctuation_marks)
