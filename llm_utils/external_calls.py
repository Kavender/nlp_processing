...
from llm_utils.count_tokens import num_tokens_from_messages
...
def get_max_tokens(max_token, messages=None, prompt: str = "", model="gpt-3.5-turbo-0613"):
    ...
    if max_allowed < 1:
        raise ValueError("Token overflow: not enough token left!")
...
def generate_and_store_response(base_prompt, user_prompt, **kwargs):
    ...
    return {
        ...
        "model_dict": {"provider": "Openai", "engine": response.model, "model": response.object},
    }
